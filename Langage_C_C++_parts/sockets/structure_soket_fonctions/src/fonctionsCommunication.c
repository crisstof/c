...
#include <stdlib.h>
#include <string.h>
#include <unistd.h>//donne accès à l’api du SE POSIX (fork, pipe, E/S, pripitive read, write close…
...
/*créer une nouvelle socket connecté avec celle qui a demandé 
la connexion (listen)reste affecté à la demande de connexion
elle sert pour échanger des données avec le 
client(cosk_conn, reçoit adresse de l’émetteur, pointeur de type socklen_t
*/
  ...
  err = recv(sock_trans, &taille, sizeof(size_t), 0);//reçoit la taille de la donnée
  if (err < 0) {
